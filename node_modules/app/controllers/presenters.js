...
Presenters = function() {
	var Presenters = function(task, channel, user, isDM){
		this._task = task;
	...
	Presenters.prototype.handleDataFromAPI = function(data){
		...
		if (!hasError && this.reminderShouldBeSet(this._task["task"])) {
			this.setReminder(this._task["task"], this._channel, this._user, this._isDM);
		} else if (hasError) {
			return hasError;
		}
	}
	...
	Presenters.prototype.sendResponseToUser = function(data) {
		...
		this._channel.send(this._enums.preProcessString(data));
	}

	Presenters.prototype.setReminder = function(task, channel, user, isDM) {
		this._reminder = new Reminder();
		this._reminderSet = this._reminder.setReminder(task, channel, user, isDM);

		if (this._reminderSet) {
