...
CueBot = function() {
	...
	CueBot.prototype.handleReq = function (message, user, channel) {
		...
		try {
			this._isDM = this._enums.checkForDM(this._channel, this._message);
			if (this._message.type === 'message') {
				if (this._isDM) {
					// Message is DM. Route appropriately ...
					var presenter = new Presenters(this._task, this._channel, this._user);
					this.routeReq(this._user, this._message, presenter.handleDataFromAPI);
				}
	...
	CueBot.prototype.routeReq = function(user, message, callback) {
		...
		return this.sendToRoute(this._control, this._params, callback);
		}
	}
	...
	CueBot.prototype.setAllReminders = function(slack) {
		try {
			...
			this.routeReq(user, bot_response, function(all_tasks){
				this._reminder = new Reminder(slack);
